...
class PolicyContractTests(unittest.TestCase):
    ...
    def test_audit_reports_missing_common_plan_fallback(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            dotnix = root / "dotnix" / "config.d/opencode/agents"
            templates = root / "Templates" / "components/agent-core/.opencode/agents"
            dotnix.mkdir(parents=True)
            templates.mkdir(parents=True)
            models = self.docs["models"]["models"]
            fallbacks = self.docs["fallback"]["fallbacks"]
            for profile, directory in (("global", dotnix), ("agent-core", templates)):
                for role, assignment in self.docs[profile]["assignments"].items():
                    model = models[assignment["primary_model"]]["id"]
                    (directory / f"{role}.md").write_text(f"---\nmodel: {model}\n---\n", encoding="utf-8")
            ...
            binding_path = root / "Templates" / "components/agent-core/.automation/model-fallback.toml"
